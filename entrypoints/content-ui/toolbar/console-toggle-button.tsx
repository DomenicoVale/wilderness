...
export const ConsoleToggleButton = ({ isOpen, count }: ConsoleToggleButtonProps) => {
  ...
  return (
    <Button
      ...
      className={cn(isOpen && "bg-primary text-primary-foreground hover:bg-primary/90")}
    >
      {isOpen ? "Close Console" : "Console"}
      {count > 0 ? (
        <span className="ml-1 rounded bg-muted px-1 text-xs text-muted-foreground">{count > 99 ? "99+" : count}</span>
      ) : null}
    </Button>
  );
};
